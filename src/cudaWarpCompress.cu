...
constexpr uint64_t META_DATA_BYTES = 36;

CudaWarpCompress::CudaWarpCompress(CudaConfig cudaConfig) : cuda_config(std::move(cudaConfig)) {
}

CudaWarpCompress::CudaWarpCompress() = default;

CudaWarpCompress::~CudaWarpCompress()
= default;

namespace WarpCompressor {
    ...
    runCompactKernel(const unsigned char *const __restrict__ comp_buff,
        ...
        // fixed region size, elements * 4 bytes * 3 parts
        constexpr uint64_t BLOCK_BYTES = ELEMS_PER_BLOCK * 4 * 3;
    ...
    __global__ void
    runCompressKernel(const float *const __restrict__ d_x, const float *const __restrict__ d_y,
                      const float *const __restrict__ d_z,
                      unsigned char *const __restrict__ comp_data,
                      volatile uint64_t *const __restrict__ loc_offset,
                      uint32_t seg_size, uint64_t coordinate_num, float eb) {
        using BlockRadixSort = cub::BlockRadixSort<uint32_t, BLOCK_SIZE, CHUNK_SIZE, uint32_t>;
        union SharedMemory {
            // Temporary storage for sorting
            typename BlockRadixSort::TempStorage sort_temp_storage;
            uint32_t boundary_idx[ELEMS_PER_BLOCK];
            uint32_t b_delta_bids[ELEMS_PER_BLOCK];
            uint32_t b_rel_pos[BLOCK_SIZE][CHUNK_SIZE + 1];
        };
        __shared__ SharedMemory smem;

        __shared__ float min_max[6];
        __shared__ uint32_t outlier;
        ...
        uint32_t local_block_ids[CHUNK_SIZE];
        uint32_t local_rel_pos[CHUNK_SIZE];
        ...
        float x_coords[CHUNK_SIZE];
        float y_coords[CHUNK_SIZE];
        float z_coords[CHUNK_SIZE];
...
#pragma unroll
        for (uint32_t i = 0; i < BLOCK_SIZE; ++i) {
            ...
            if (valid) {
                local_min.x = fminf(local_min.x, x);
                local_min.y = fminf(local_min.y, y);
                local_min.z = fminf(local_min.z, z);

                local_max.x = fmaxf(local_max.x, x);
                local_max.y = fmaxf(local_max.y, y);
                local_max.z = fmaxf(local_max.z, z);
            }
        }
        ...
        float inv_2eb = 1.0f / (2.0f * eb);
        uint32_t block_num_x = 0, block_num_y = 0;
        ...
        if (tid == 0) {
            ...
            block_num_x =
                    static_cast<uint32_t>(ceilf((local_max.x - local_min.x) / (2 * seg_size * eb))) + 1;
            block_num_y =
                    static_cast<uint32_t>(ceilf((local_max.y - local_min.y) / (2 * seg_size * eb))) + 1;
...
#pragma unroll
        for (uint32_t i = 0; i < BLOCK_SIZE; ++i) {
            ...
            float x = valid ? x_coords[i] : 0.0f;
            float y = valid ? y_coords[i] : 0.0f;
            float z = valid ? z_coords[i] : 0.0f;

            // force to FFMA: a * b + c
            float quant_x_f = __fmaf_rn(x, inv_2eb, -local_min.x * inv_2eb);
            float quant_y_f = __fmaf_rn(y, inv_2eb, -local_min.y * inv_2eb);
            float quant_z_f = __fmaf_rn(z, inv_2eb, -local_min.z * inv_2eb);
            uint32_t quant_y = static_cast<uint32_t>(quant_y_f);
            uint32_t quant_x = static_cast<uint32_t>(quant_x_f);
            uint32_t quant_z = static_cast<uint32_t>(quant_z_f);

            // use shift as division ; & 0x7FFFFFFF remove highest sign bit
            uint32_t block_id = ((quant_x >> seg_shift) +
                                     ((quant_y >> seg_shift) * block_num_x) +
                                     ((quant_z >> seg_shift) * block_num_x * block_num_y)) & 0x7FFFFFFF;
            // use & seg_mask as modular
            uint32_t relative_pos = ((quant_x & seg_mask) +
                                         ((quant_y & seg_mask) * seg_size) +
                                         ((quant_z & seg_mask) * seg_size * seg_size)) & 0x7FFFFFFF;


            local_block_ids[i] = valid ? block_id : UINT_MAX;
            local_rel_pos[i] = valid ? relative_pos : UINT_MAX;
        ...
        uint32_t thread_last_item = local_block_ids[CHUNK_SIZE - 1];
        uint32_t pre_thread_last_item = __shfl_up_sync(0xffffffff, thread_last_item, 1);

#pragma unroll
        for (uint32_t i = 0; i < CHUNK_SIZE; i++) {
            uint32_t local_idx = tid * CHUNK_SIZE + i;
            uint32_t cur_val = local_block_ids[i];
            uint32_t prev_val = (i == 0) ? pre_thread_last_item : local_block_ids[i - 1];
...
#pragma unroll
        ...
        uint32_t local_delta_bids[CHUNK_SIZE];
        uint32_t cur_thread_last_bid = local_block_ids[CHUNK_SIZE - 1];
        uint32_t pre_thread_last_bid = __shfl_up_sync(0xffffffff, cur_thread_last_bid, 1);
        uint32_t pre_local_bid = 0;
        int32_t local_delta_bid_count = 0;
#pragma unroll
        for (uint32_t i = 0; i < CHUNK_SIZE; i++) {
            if (sbuf[i][tid] == 0) continue;

            uint32_t b_id = local_block_ids[i];
            uint32_t delta = 0;
        ...
        uint32_t b_delta_bids_bytes =
                (((((unique_num + 7) & ~7U) * local_max_delta_block_id_bit) >> 3) + 3) & ~3U;
        uint32_t b_count_bytes =
                (((((unique_num + 7) & ~7U) * local_max_cnt_bit) >> 3) + 3) & ~3U;
        uint32_t b_rel_pos_bytes = (((ELEMS_PER_BLOCK * local_max_rel_pos_bit) >> 3) + 3) & ~3U;
...
#pragma unroll
        for (int32_t i = threadIdx.x; i < 6; i += 32) {
            reinterpret_cast<float *>(meta_dst)[i] = min_max[i];
        }

        if (threadIdx.x == 6) {
            reinterpret_cast<uint32_t *>(meta_dst)[6] = outlier;
        }

        if (threadIdx.x >= 7 && threadIdx.x <= 10) {
            reinterpret_cast<uint16_t *>(meta_dst + 28)[threadIdx.x - 7] = max_bits_unique_num[threadIdx.x - 7];
        }
        ...
        uint64_t base_idx = warp_id * ELEMS_PER_BLOCK * 4 * 3ULL + meta_offset;
...
#pragma unroll
            for (int32_t i = 0; i < CHUNK_SIZE; ++i) {
                uint32_t val = smem.b_rel_pos[i][tid];
    ...
    __global__ void
    runCompressKernel2D(const float *const __restrict__ d_x, const float *const __restrict__ d_y,
                      unsigned char *const __restrict__ comp_data,
                      volatile uint64_t *const __restrict__ loc_offset,
                      uint32_t seg_size, uint64_t coordinate_num, float eb) {
        using BlockRadixSort = cub::BlockRadixSort<uint32_t, BLOCK_SIZE, CHUNK_SIZE, uint32_t>;
...
#pragma unroll
    ...
    __global__ void
    runDecompressKernel(float *decData_x, float *decData_y, float *decData_z,
...
//        __shared__ uint32_t d_xyz_block_num[3];
        ...
        __shared__ uint32_t b_buff_block_id[BLOCK_SIZE * CHUNK_SIZE];
        __shared__ uint32_t b_buff_counts_pos[BLOCK_SIZE * CHUNK_SIZE];
        __shared__ uint16_t max_bits_unique_num[4];
        ...
        uint32_t block_num_x, block_num_y;
        if (tid == 0) {
            const unsigned char *meta_src = comp_data + warp_id * META_DATA_BYTES;
            const float *f_src = reinterpret_cast<const float *>(meta_src);
            for (int32_t i = 0; i < 6; ++i) {
                min_max[i] = f_src[i];
            }

            outlier = reinterpret_cast<const uint32_t *>(meta_src)[6];

            const uint16_t *u16_src = reinterpret_cast<const uint16_t *>(meta_src + 28);
            for (int32_t i = 0; i < 4; ++i) {
                max_bits_unique_num[i] = u16_src[i];
            }

            block_num_x =
                    static_cast<uint32_t>(ceilf((min_max[3] - min_max[0]) / (2 * seg_size * eb))) + 1;
            block_num_y =
                    static_cast<uint32_t>(ceilf((min_max[4] - min_max[1]) / (2 * seg_size * eb))) + 1;
        }
        block_num_x = __shfl_sync(0xffffffff, block_num_x, 0);
        block_num_y = __shfl_sync(0xffffffff, block_num_y, 0);
        ...
        uint32_t b_delta_bids_bytes =
                (((((max_bits_unique_num[3] + 7) & ~7U) * max_bits_unique_num[0]) >> 3) + 3) & ~3U;
        uint32_t b_count_bytes =
                (((((max_bits_unique_num[3] + 7) & ~7U) * max_bits_unique_num[1]) >> 3) + 3) & ~3U;
...
#pragma unroll
        ...
        uint32_t local_buffs[CHUNK_SIZE];
        ...
        // SOA input
        float *x_ptr = decData_x + global_block_start;
        float *y_ptr = decData_y + global_block_start;
        float *z_ptr = decData_z + global_block_start;

        int32_t seg_shift = __ffs(seg_size) - 1;
        uint32_t seg_mask = seg_size - 1;
        for (uint64_t i = tid; i < total_valid_points; i += BLOCK_SIZE) {
            uint32_t rel_pos = b_buff_counts_pos[i];
            uint32_t blk_id = b_buff_block_id[i];

            uint32_t block_x = block_num_x;
            uint32_t block_y = block_num_y;

            uint32_t bx = blk_id % block_x;
            uint32_t by = (blk_id / block_x) % block_y;
            uint32_t bz = blk_id / (block_x * block_y);

            uint32_t rx = rel_pos & seg_mask;
            uint32_t ry = (rel_pos >> seg_shift) & seg_mask;
            uint32_t rz = rel_pos >> (seg_shift * 2);

            uint32_t qx = bx * seg_size + rx;
            uint32_t qy = by * seg_size + ry;
            uint32_t qz = bz * seg_size + rz;

            float x = min_max[0] + ((qx << 1) | 1) * eb;
            float y = min_max[1] + ((qy << 1) | 1) * eb;
            float z = min_max[2] + ((qz << 1) | 1) * eb;
    ...
    __global__ void
    runDecompressKernel2D(float *decData_x, float *decData_y,
                        unsigned char *const __restrict__ comp_data, const uint64_t *const __restrict__ comp_offset,
                        uint32_t seg_size, uint64_t coordinate_num, float eb) {
        __shared__ float min_max[6];
        __shared__ uint32_t outlier;
...
#pragma unroll
    ...
    __global__ void sampleMinMaxXYZKernel(const float *d_x, const float *d_y, const float *d_z,
                                          float *d_min_x, float *d_min_y, float *d_min_z, float *d_max_x, float *d_max_y,
                                          float *d_max_z, int32_t sample_length, uint64_t sample_stride, uint64_t N) {
        extern __shared__ float sdata[];
        float *smin_x = sdata;
        float *smax_x = &smin_x[sample_length];
        float *smin_y = &smax_x[sample_length];
        float *smax_y = &smin_y[sample_length];
        float *smin_z = &smax_y[sample_length];
        float *smax_z = &smin_z[sample_length];
...
#pragma unroll
        for (int32_t stride = sample_length / 2; stride > 0; stride >>= 1) {
            if (tid < stride) {
                smin_x[tid] = fminf(smin_x[tid], smin_x[tid + stride]);
                smax_x[tid] = fmaxf(smax_x[tid], smax_x[tid + stride]);

                smin_y[tid] = fminf(smin_y[tid], smin_y[tid + stride]);
                smax_y[tid] = fmaxf(smax_y[tid], smax_y[tid + stride]);

                smin_z[tid] = fminf(smin_z[tid], smin_z[tid + stride]);
                smax_z[tid] = fmaxf(smax_z[tid], smax_z[tid + stride]);
            }
            __syncthreads();
        }
    ...
    __global__ void sampleMinMaxXYZKernel2D(const float *d_x, const float *d_y,
                                            float *d_min_x, float *d_min_y, float *d_max_x, float *d_max_y,
                                            int32_t sample_length, uint64_t sample_stride, uint64_t N) {
        extern __shared__ float sdata[];
        float *smin_x = sdata;
        float *smax_x = &smin_x[sample_length];
        float *smin_y = &smax_x[sample_length];
...
#pragma unroll
    ...
    __global__ void reduceMinMax(const float *min_x, const float *max_x,
                                 const float *min_y, const float *max_y,
                                 const float *min_z, const float *max_z,
                                 float *out_block_results,
                                 int32_t N) {
        extern __shared__ float sdata[];
        float *s_min_x = sdata + 0 * blockDim.x;
        float *s_max_x = sdata + 1 * blockDim.x;
        float *s_min_y = sdata + 2 * blockDim.x;
        float *s_max_y = sdata + 3 * blockDim.x;
        float *s_min_z = sdata + 4 * blockDim.x;
        float *s_max_z = sdata + 5 * blockDim.x;

        int32_t tid = threadIdx.x;
        int32_t i = blockIdx.x * blockDim.x + tid;

        s_min_x[tid] = s_min_y[tid] = s_min_z[tid] = FLT_MAX;
        s_max_x[tid] = s_max_y[tid] = s_max_z[tid] = FLT_MIN;
        ...
        if (tid == 0) {
            atomicMinFl(&out_block_results[0], s_min_x[0]);
            atomicMaxFl(&out_block_results[1], s_max_x[0]);
            atomicMinFl(&out_block_results[2], s_min_y[0]);
            atomicMaxFl(&out_block_results[3], s_max_y[0]);
            atomicMinFl(&out_block_results[4], s_min_z[0]);
            atomicMaxFl(&out_block_results[5], s_max_z[0]);
        }
    }

    __global__ void reduceMinMax2D(const float *min_x, const float *max_x,
                                   const float *min_y, const float *max_y,
                                   float *out_block_results,
                                   int32_t N) {
        extern __shared__ float sdata[];
        ...
        s_min_x[tid] = s_min_y[tid] = FLT_MAX;
        s_max_x[tid] = s_max_y[tid] = FLT_MIN;
...
void CudaWarpCompress::computeSegSize(float *d_x, float *d_y, float *d_z, float *d_cache, CudaConfig &cuda_config) {
    ...
    uint32_t shared_mem_bytes = sample_length * 6 * sizeof(float);

    float *d_min_x = d_cache;
    float *d_min_y = d_cache + num_blocks;
    float *d_min_z = d_cache + num_blocks * 2;

    float *d_max_x = d_cache + num_blocks * 3;
    float *d_max_y = d_cache + num_blocks * 4;
    float *d_max_z = d_cache + num_blocks * 5;

    float *d_output_minmax;
    float init_vals[6] = {
            FLT_MAX, -FLT_MAX,
            FLT_MAX, -FLT_MAX,
            FLT_MAX, -FLT_MAX
    };
    cudaMalloc(&d_output_minmax, sizeof(float) * 6);
    ...
    if (cuda_config.dim == 3) {
        WarpCompressor::sampleMinMaxXYZKernel<<<num_blocks, sample_length, shared_mem_bytes>>>(
                d_x, d_y, d_z,
                d_min_x, d_min_y, d_min_z,
                d_max_x, d_max_y, d_max_z,
                sample_length, stride_between_samples, cuda_config.coordinate_num);

        WarpCompressor::reduceMinMax<<<block_reduce_blocks, block_reduce_threads_per_block, 6 * block_reduce_threads_per_block *
                                                                            sizeof(float)>>>(
                d_min_x, d_max_x,
                d_min_y, d_max_y,
                d_min_z, d_max_z,
        ...
        cudaMemcpy(h_output, d_output_minmax, sizeof(float) * 6, cudaMemcpyDeviceToHost);
    } else {
        WarpCompressor::sampleMinMaxXYZKernel2D<<<num_blocks, sample_length, shared_mem_bytes>>>(
                d_x, d_y,
                d_min_x, d_min_y,
                d_max_x, d_max_y,
                sample_length, stride_between_samples, cuda_config.coordinate_num);

        WarpCompressor::reduceMinMax2D<<<block_reduce_blocks, block_reduce_threads_per_block, 6 * block_reduce_threads_per_block *
                                                                            sizeof(float)>>>(
                d_min_x, d_max_x,
                d_min_y, d_max_y,
                d_output_minmax,
        ...
        cudaMemcpy(h_output, d_output_minmax, sizeof(float) * 4, cudaMemcpyDeviceToHost);
        h_output[4] = 0;
        h_output[5] = 0;
    }

    float min_x = h_output[0];
    float max_x = h_output[1];
    float min_y = h_output[2];
    float max_y = h_output[3];
    float min_z = h_output[4];
    float max_z = h_output[5];
    ...
    float variance = 0.0f;
    if (cuda_config.dim == 3) {
        variance = ((dx - mean) * (dx - mean) + (dy - mean) * (dy - mean) + (dz - mean) * (dz - mean)) / 3.0f;
    } else {
        variance = ((dx - mean) * (dx - mean) + (dy - mean) * (dy - mean) + (dz - mean) * (dz - mean)) / 2.0f;
    }

    float std_dev = sqrtf(variance);
    float cubic_ratio = std_dev / mean;
//    printf("Cubic ratio: %.6f\n", cubic_ratio);
    ...
    uint64_t max_blocks = static_cast<uint64_t>(ceilf(max_range / (2 * cuda_config.seg_size * cuda_config.eb))) + 1;
    uint64_t total_block_id = cuda_config.dim == 3 ? max_blocks * max_blocks * max_blocks : max_blocks * max_blocks;
    bool adjusted_seg = total_block_id > UINT32_MAX;

    if (adjusted_seg == 1) {
        uint32_t temp_segment_size = cuda_config.seg_size;
        for (int32_t i = 0; i < 11; i++) {
            temp_segment_size *= 2;
            max_blocks = static_cast<uint64_t>(ceilf(max_range / (2 * temp_segment_size *  cuda_config.eb))) + 1;
            total_block_id = cuda_config.dim == 3 ? max_blocks * max_blocks * max_blocks : max_blocks * max_blocks;
            if (total_block_id <= UINT32_MAX) {
                break;
            }
        }
...
void CudaWarpCompress::compress(void *d_xyz, unsigned char *comp_data, uint64_t *comp_data_size,
                                const std::string &comp_type) {
    auto* xyz_f = reinterpret_cast<float*>(d_xyz);
    compressImpl(xyz_f, comp_data, comp_data_size, comp_type);
}

void CudaWarpCompress::decompress(void *decData_x, void *decData_y, void *decData_z, unsigned char *d_compressed_data,
                                  const std::string &nvcomp_type) {
    auto* x = reinterpret_cast<float*>(decData_x);
    auto* y = reinterpret_cast<float*>(decData_y);
    auto* z = reinterpret_cast<float*>(decData_z);
    decompressImpl(x, y, z, d_compressed_data, nvcomp_type);
}

void
CudaWarpCompress::compressImpl(float *d_xyz, unsigned char *d_comp_data, uint64_t *comp_size,
...
//        cascaded_opts.internal_chunk_bytes = nvcomp_chunk_size;
    ...
    float *d_x = d_xyz;
    float *d_y = d_xyz + cuda_config.coordinate_num;
    float *d_z = d_xyz + 2 * cuda_config.coordinate_num;

    float *d_cache = reinterpret_cast<float *>(d_comp_data);
    // auto select seg size
    computeSegSize(d_x, d_y, d_z, d_cache, cuda_config);
    if (cuda_config.dim == 3) {
        // get equal length of compressed data
        WarpCompressor::runCompressKernel<<<gridDim, blockDim>>>(d_x, d_y, d_z, d_comp_data, d_loc_offset,
                cuda_config.seg_size,
                cuda_config.coordinate_num,
                cuda_config.eb);
    } else {
        // get equal length of compressed data
        WarpCompressor::runCompressKernel2D<<<gridDim, blockDim>>>(d_x, d_y, d_comp_data, d_loc_offset,
                cuda_config.seg_size,
                cuda_config.coordinate_num,
                cuda_config.eb);
    }
    ...
    cudaMemcpy(d_xyz_out + 16, &cuda_config.eb, 4, cudaMemcpyHostToDevice);
    cudaMemcpy(d_xyz_out + 20, &cuda_config.seg_size, 4, cudaMemcpyHostToDevice);
    cudaMemcpy(d_xyz_out + 24, d_cmp_offset, prefix_sum_offset, cudaMemcpyDeviceToDevice);
    ...
    WarpCompressor::runCompactKernel<<<grid_size, 256>>>(d_comp_data + meta_offset, d_xyz_out + parameter_size +
                                                                                    prefix_sum_offset +
...
void
CudaWarpCompress::decompressImpl(float *decData_x, float *decData_y, float *decData_z, unsigned char *d_compressed_data,
                             const std::string &nvcomp_type) {
    // Mark the start of the compress function
...
//            cascaded_opts.internal_chunk_bytes = nvcomp_chunk_size;
    ...
    cudaMemcpy(&h_eb, d_compressed_data + 16, 4, cudaMemcpyDeviceToHost);
    cudaMemcpy(&h_seg_size, d_compressed_data + 20, 4, cudaMemcpyDeviceToHost);
    ...
    if (dim == 3) {
        // decompress
        WarpCompressor::runDecompressKernel<<<gridDim, blockDim>>>(decData_x, decData_y, decData_z, d_compressed_data +
                                                                                                    parameter_size +
        ...
        // decompress
        WarpCompressor::runDecompressKernel2D<<<gridDim, blockDim>>>(decData_x, decData_y, d_compressed_data +
                                                                                                        parameter_size +
                                                                                                        prefix_sum_offset, d_cmp_offset,
                h_seg_size,
                h_coordinate_num,
